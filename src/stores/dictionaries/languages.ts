...
import { TLanguageModel } from "@/models/dictionaries/language";
...
export const useLanguagesStore = create(
  persist<{
    ...
    error: string | null;
    total: number | null;
    items: TLanguageModel[] | null;
    getItems: (filters?: TModelFilters) => TLanguageModel[] | null;
    ...
    (set, get) => ({
      ...
      fetchItems: async (filters?: TModelFilters) => {
        ...
        try {
          ...
          const response = await http("/ws/rest/com.axelor.apps.base.db.Language/search", {
            method: "POST",
