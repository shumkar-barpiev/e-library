...
import { TCompanyModel } from "@/models/dictionaries/company";
...
export const useCompaniesStore = create(
  persist<{
    ...
    error: string | null;
    total: number | null;
    items: TCompanyModel[] | null;
    getItems: (filters?: TModelFilters) => TCompanyModel[] | null;
    ...
    (set, get) => ({
      ...
      fetchItems: async (filters?: TModelFilters) => {
        ...
        try {
          ...
          const response = await http("/ws/rest/com.axelor.apps.base.db.Company/search", {
            method: "POST",
